...
MADWIFI_DEPENDENCIES = uclibc linux
...
$(MADWIFI_DIR)/tools/$(MADWIFI_BINARY): $(MADWIFI_DIR)/.configured
	$(MAKE) -C $(MADWIFI_DIR) KERNELPATH=$(MADWIFI_KERNEL_PATH) CROSS_COMPILE=$(TARGET_CROSS)
...
$(TARGET_DIR)/usr/bin/$(MADWIFI_TARGET_BINARY): $(MADWIFI_DIR)/tools/$(MADWIFI_BINARY)
	...
	$(MAKE) -C $(MADWIFI_DIR) KERNELPATH=$(MADWIFI_KERNEL_PATH) CROSS_COMPILE=$(TARGET_CROSS)  KMODPATH=$(TARGET_DIR)/lib/modules/$(MADWIFI_KERNEL_VERSION) install-modules
	/sbin/depmod -ae $(MADWIFI_KERNEL_VERSION) -b $(TARGET_DIR)
	touch $@
