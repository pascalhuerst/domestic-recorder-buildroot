...
$(REPORT_DAEMON_DIR)/.bzr:
	...
	if ! test -d $(REPORT_DAEMON_DIR)/.bzr; then \
		...
		mkdir -p raumfeld-report-daemon-$(REPORT_DAEMON_VERSION); \
	 	$(call qstrip,$(BR2_BZR)) co -q --lightweight $(BR2_PACKAGE_RAUMFELD_REPOSITORY)/raumfeld-report-daemon/$(REPORT_DAEMON_VERSION) raumfeld-report-daemon-$(REPORT_DAEMON_VERSION)) \
	fi
	touch -c $@
